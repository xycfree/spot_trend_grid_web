...
if __name__ == '__main__':
    ...
    data = {
        "symbol": "ETHUSDT",
        "total_money": 6472,  # 总投入资金
        "order_interval": 0.01,  # 下单间距%
        "order_interval_increase": 0.3,  # 下单递增间距%
        "initial_invest_capital": 74,  # 初始投入资金
        "capital_interval_increase": 0.25,  # 资金递增幅度%
        "profit_ratio": 0.05,  # 利润比例%
        "price_precision": 2,  # 价格精度
        "amount_precision": 5,  # 数量精度
        "buy_procedure_fee": 0.001,  # 买入手续费
        "sell_procedure_fee": 0.001,  # 卖出手续费
    }
    loop_run(data)
