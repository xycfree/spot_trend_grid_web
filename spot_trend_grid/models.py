...
class SpotConfigModel(models.Model):
    # DecimalField中的（max_digits的范围，decimal_places=小数的长度）
    # decimal.InvalidOperation: [<class 'decimal.InvalidOperation'>] 这个报错其实是数据库的数据长度超过了原本的长度
    ...
    grid_sell_price = models.DecimalField(decimal_places=8, max_digits=16, help_text="当前止盈价(你的当前仓位卖出价)",
                                          verbose_name="当前止盈价")
    last_price = models.CharField(max_length=256, null=True, blank=True, default="", help_text="最新买入价",
                                  verbose_name="最新买入价")
    step = models.IntegerField(help_text="当前仓位(0:仓位为空)", verbose_name="当前仓位")
    profit_ratio = models.CharField(max_length=256, null=True, blank=True, default="",
                                    help_text="止盈比率列表(卖出价调整比率。如：设置为5,即为5%，当前买入价为100，那么下次卖出价为105),为空则计算atr波动率",
                                    verbose_name="止盈比率")
    double_throw_ratio = models.CharField(max_length=256, null=True, blank=True, default="",
                                          help_text="补仓比率列表(买入价调整比率。如：设置为5,即为5%当前买入价为100，那么下次买入价为95),为空则计算atr波动率",
                                          verbose_name="补仓比率")
    coin_type = models.CharField(max_length=16, help_text="交易对(如：BTC 填入 BTCUSDT/BTCBUSD)",
                                 verbose_name="交易对")
    quantity = models.CharField(max_length=256, help_text="交易数量列表", null=True, blank=True, verbose_name="交易数量列表", default="")
    price_quantity = models.CharField(max_length=256, help_text="交易金额列表(第一手买入1,第二手买入2...),例如:10,15,20",
                                      verbose_name="交易金额")
    free = models.DecimalField(decimal_places=8, max_digits=16, help_text="可用货币数量", verbose_name="可用数量", null=True,
                               blank=True, default=0)
    max_count = models.IntegerField(help_text="连续买入而不卖出的最大次数", verbose_name="连续买入而不卖出的最大次数", default=5)
    min_num = models.IntegerField(help_text="交易金额最小位数长度,如0.00001,则填入5", verbose_name="交易金额最小位数长度", default=5)
    current_num = models.IntegerField(help_text="当前连续买入次数", verbose_name="当前连续买入次数", default=0)
    current_income = models.DecimalField(decimal_places=6, max_digits=16, help_text="当前收益", verbose_name="当前收益",
                                         default=0)
...
class BatchOrderDetailModel(models.Model):
    ...
    order_id = models.DecimalField(verbose_name="订单id", max_digits=16, decimal_places=0, default=0, null=True,
                                   blank=True)
